...
export async function orchestrateChat(
  ...
  let url = `${API_BASE || '/api/v1'}/chat/orchestrate?token=${encodeURIComponent(await getToken())}`;
  ...
  if (response.status === 403) {
    clearTokenCache();
    url = `${API_BASE || '/api/v1'}/chat/orchestrate?token=${encodeURIComponent(await getToken())}`;
    response = await fetch(url, reqConfig);
  }
...
export async function truncateConversation(
  ...
  messageId: string,
  keepInclusive: boolean
): Promise<{ success: boolean }> {
  return apiPost(`/chat/conversations/${encodeURIComponent(conversationId)}/truncate`, {
    message_id: messageId,
    keep_inclusive: keepInclusive,
  });
}
...
export async function recordFeedback(
  ...
  return apiPost(`/chat/messages/${encodeURIComponent(messageId)}/feedback?conversation_id=${encodeURIComponent(conversationId)}`, {
    feedback,
  });
}
