    ...
    <div className="flex flex-col h-full bg-black/40 backdrop-blur-xl border-l border-white/5">
      <div className="p-4 border-b border-white/10 flex items-center justify-between bg-white/5">
        ...
        <div className="flex items-center gap-2">
          <div className={clsx(
            ...
            workflowState === 'reasoning' ? "bg-indigo-500 animate-pulse-glow" : "bg-gray-500"
          )} />
        </div>
      </div>
      ...
      <div className="p-4 space-y-6 overflow-y-auto">
        ...
        <div className="glass-panel p-4 rounded-xl relative overflow-hidden group">
          ...
          <div className="text-xs text-gray-500 font-medium uppercase tracking-wider mb-1">Status</div>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-bold text-white capitalize">{workflowStage}</span>
            <span className="text-xs text-indigo-400 font-mono">[{workflowState}]</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <MetricCard icon={<ShieldAlert size={14} />} label="Session Cost" value={`$${totalSessionCost.toFixed(4)}`} className="col-span-2 border-indigo-500/30 bg-indigo-500/5 text-indigo-200" />
        </div>

        {/* Reasoning Trace */}
        <div className="mt-4">
          <div className="text-xs text-gray-500 font-medium uppercase tracking-wider mb-2 flex justify-between">
            <span>Live Reasoning</span>
            <span className="font-mono text-[10px] bg-white/10 px-1.5 py-0.5 rounded text-gray-400">TRACE</span>
          </div>
